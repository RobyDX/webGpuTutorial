 ...
 * Render to Texture
 * 
 * Rendering su texture per realizzare effetti a più step
 */

import { baseRendering } from "../utility/baseRendering";
import { Mat4, mat4 } from 'wgpu-matrix'

export class Tutorial12 extends baseRendering {
    ...
    //coppia di render target e depth texture aggiuntivi
    private _renderTarget2: GPUTexture = null!;
    private _depthTexture2: GPUTexture = null!;

    //binding per utilizzare il render target
    private _textureBindGroup2: GPUBindGroup = null!;
    ...
    async init() {
        ...
        //creazione di render target ausiliari

        //creazione di una texture da utilizzare il render target
        this._renderTarget2 = device.createTexture({
            size: [512, 512],
            format: 'bgra8unorm',
        ...
        //creazione di una texture da utilizzare per lo ZBuffer
        this._depthTexture2 = device.createTexture({
            size: [512, 512],
            format: 'depth24plus',
            usage: GPUTextureUsage.RENDER_ATTACHMENT,
        });

        this._textureBindGroup2 = device.createBindGroup({
            layout: this._pipeline.getBindGroupLayout(1),
            entries: [
                { binding: 0, resource: this._sampler },
                { binding: 1, resource: this._renderTarget2.createView() }
            ],
        });
    }
    ...
    draw() {
        ...
        {
            //definisce le caratteristiche del render pass per renderizzare su una texture
            const renderPassDescriptor: GPURenderPassDescriptor = {
                colorAttachments: [
                    {
                        view: this._renderTarget2.createView(),
                        clearValue: [0, 0, 1, 0],
                        loadOp: 'clear',
                        storeOp: 'store',
                    },
                ...
                depthStencilAttachment: {
                    view: this._depthTexture2.createView(),
                    depthClearValue: 1.0,
                    depthLoadOp: 'clear',
                    depthStoreOp: 'store',
                }
        ...
        //ultimo step
        {
...

            {
                ...
                pass.setBindGroup(1, this._textureBindGroup2);
