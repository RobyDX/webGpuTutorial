 ...
 * MSAA
 * 
 * Miglioramento della resa grafica tramite Multi Sample Antialasing
 */

import { baseRendering } from "../utility/baseRendering";
import { Mat4, mat4 } from 'wgpu-matrix'

export class Tutorial11 extends baseRendering {
    ...
    //render pipeline
    private _pipelineMultiSample: GPURenderPipeline = null!;
    private _pipelineNoSample: GPURenderPipeline = null!;
    ...
    private _depthTextureMultiSample: GPUTexture = null!;
    private _depthTextureNoSample: GPUTexture = null!;
    ...
    private multipleSampleOn: boolean = true;
    ...
    async init() {
        ...
        document.querySelector("p")!.innerText = this.multipleSampleOn ? "MSAA Attivo" : "MSAA Non Attivo";
        canvas.onclick = () => {
            this.multipleSampleOn = !this.multipleSampleOn;

            document.querySelector("p")!.innerHTML = this.multipleSampleOn ? "MSAA Attivo" : "MSAA Non Attivo";
        };
        ...
        //creazione di una texture da utilizzare per lo ZBuffer per il MultiSample
        this._depthTextureMultiSample = device.createTexture({
        ...
        //creazione di una texture da utilizzare per lo ZBuffer senza MultiSample
        this._depthTextureNoSample = device.createTexture({
            size: [canvas.width, canvas.height],
            format: 'depth24plus',
            usage: GPUTextureUsage.RENDER_ATTACHMENT
        });

        //texture per il render target multisample
        this.multisampleTexture = this._device.createTexture({
            format: "bgra8unorm",
            usage: GPUTextureUsage.RENDER_ATTACHMENT,
            size: [canvas.width, canvas.height],
            sampleCount: this.sampleCount,
        ...
        //crea la pipeline con multisample
        this._pipelineMultiSample = device.createRenderPipeline({
        ...
        //crea la pipeline senza multisample
        this._pipelineNoSample = device.createRenderPipeline({
            layout: this._device.createPipelineLayout({
                bindGroupLayouts: [
                    this._device.createBindGroupLayout({
                        entries: [{ binding: 0, visibility: GPUShaderStage.VERTEX | GPUShaderStage.FRAGMENT, buffer: { type: "uniform" } }]
        ...
        this._bindGroup = device.createBindGroup({
            layout: this._device.createBindGroupLayout({
                entries: [
    ...
    draw() {
        ...
        //definisce le caratteristiche del render pass per il MSAA
        let pass: GPURenderPassEncoder;

        if (this.multipleSampleOn) {
            const renderPassDescriptor: GPURenderPassDescriptor = {
                ...
                depthStencilAttachment: {
                    view: this._depthTextureMultiSample.createView(),
                    depthClearValue: 1.0,
                    depthLoadOp: 'clear',
                    depthStoreOp: 'store',
                }
            ...
            pass.setPipeline(this._pipelineMultiSample);
            ...
            const renderPassDescriptor: GPURenderPassDescriptor = {
                ...
                depthStencilAttachment: {
                    view: this._depthTextureNoSample.createView(),
                    depthClearValue: 1.0,
                    depthLoadOp: 'clear',
                    depthStoreOp: 'store',
                }
            ...
            //imposta la pipeline da eseguire
            pass.setPipeline(this._pipelineNoSample);
        }
    ...
    destroy(): void {
        ...
        this._depthTextureMultiSample.destroy();
        this._depthTextureNoSample.destroy();
