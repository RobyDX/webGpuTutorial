/**
 * Testo su una Texture
 * 
 * Utilizzare una canvas come texture per disegnare testo
 */

import { baseRendering } from "../utility/baseRendering";
import { Mat4, mat4 } from 'wgpu-matrix'

export class Tutorial18 extends baseRendering {
    ...
    //canvas offline
    private _offlineCanvas: HTMLCanvasElement = null!;
    ...
    async init() {
        ...
        //crea una canvas offline della stessa dimensione della texture
        this._offlineCanvas = document.createElement('canvas');
        this._offlineCanvas.width = this._texture.width;
        this._offlineCanvas.height = this._texture.height;

        //crea un context 2d
        this._context2D = this._offlineCanvas.getContext('2d')!;
    ...
    draw() {
        ...
        //utilizzo del context2d per pulire la canvas offline e scriverci sopra del testo
        this._context2D.fillStyle = "#0000FF";
        this._context2D.fillRect(0, 0, this._offlineCanvas.width, this._offlineCanvas.height);
        this._context2D.font = "32px Arial";
        this._context2D.fillStyle = "white";
        const d = new Date();

        this._context2D.fillText("Time", 32, 64);
        this._context2D.fillText(`${d.getDate().toString().padStart(2, '0')}:${(d.getMonth() + 1).toString().padStart(2, '0')}:${d.getFullYear().toString().padStart(2, '0')}`
            , 32, 96);
        this._context2D.fillText(`${d.getHours().toString().padStart(2, '0')}:${d.getMinutes().toString().padStart(2, '0')}:${d.getSeconds().toString().padStart(2, '0')}`
            , 32, 128);

        // si crea un command encoder che eseguirà le operazioni
        const encoder = this._device.createCommandEncoder();

        //ottiene l'immagine dalla canvas e la copia sulla texture
        createImageBitmap(this._offlineCanvas).then(data => {
            this._device.queue.copyExternalImageToTexture(
                ...
                [this._offlineCanvas.width, this._offlineCanvas.height]);
        });
...

        {
            ...
            mat4.rotateY(world, new Date().getTime() / 2000.0, world);
