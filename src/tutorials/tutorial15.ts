 ...
 * Sobel Filter
 * 
 * Esempio di utilizzo di compute shader in fase di rendering
 */

import { baseRendering } from "../utility/baseRendering";
import { Mat4, mat4 } from 'wgpu-matrix'

export class Tutorial15 extends baseRendering {
    ...
    //binding group per il sampler e per la texture per l'effetto Sobel
    private _sobelBindGroup: GPUBindGroup = null!;

    private showSobel: boolean = true;

    //compute pipeline
    private _computePipeline: GPUComputePipeline = null!;

    //binding group per il compute
    private _computeBindGroup: GPUBindGroup = null!;
...
`;
    ...
    async init() {
        ...
        //output per il compute shader
        const outputTexture = device.createTexture({
            size: { width: this._texture.width, height: this._texture.height },
            format: "rgba8unorm",
        ...
        this._sobelBindGroup = device.createBindGroup({
            layout: this._pipeline.getBindGroupLayout(1),
            entries: [
                { binding: 0, resource: this._sampler },
        ...
        //crea lo shader
        const computemodule = device.createShaderModule({ code: this.computeShaderCode });
        ...
        canvas.onclick = () => this.showSobel = !this.showSobel;
    ...
    draw() {
...

        {
            ...
            if (this.showSobel)
                pass.setBindGroup(1, this._sobelBindGroup);
            else
